...
public class WednesdayFragment extends Fragment {
    ...
    private void showCourses(String deptName, String deptField, String deptSpec, String sessionId, String semesterItem) {
        ...
        database1 = FirebaseDatabase.getInstance().getReference("IndividualTimeTable").child(deptName).child(deptField).child(deptSpec).child(semesterItem).child("Wednesday");
        ...
        FirebaseRecyclerAdapter       adapter1 = new FirebaseRecyclerAdapter<Category,WednesdayFragment.UserViewHolder>(options) {
            ...
            @Override
            protected void onBindViewHolder(@NonNull final WednesdayFragment.UserViewHolder holder, final int position, @NonNull final Category category) {
                ...
                database1.child(selected_user_id1).addListenerForSingleValueEvent(new ValueEventListener() {
                    ...
                    public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                        ...
                        if(position % 2 == 0){
                            holder.constraintLayout.setBackgroundColor(ContextCompat.getColor(getContext(), R.color.bg_login));
                        }

                        else {
                            holder.constraintLayout.setBackgroundColor(ContextCompat.getColor(getContext(), R.color.purple));
